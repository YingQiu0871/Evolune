package io.github.yuninggu.hrttracker.viewmodel
...
import io.github.yuninggu.hrttracker.utils.MahiroJsonFormat
import io.github.yuninggu.hrttracker.utils.MedicationPlanPredictor
...
class HRTViewModel(
    ...
    // 所有用药计划列表
    val allPlans: StateFlow<List<io.github.yuninggu.hrttracker.data.MedicationPlan>> = medicationPlanRepository.getAllPlans()
        .stateIn(
            scope = viewModelScope,
            started = SharingStarted.WhileSubscribed(5000),
            initialValue = emptyList()
    ...
    // 启用的用药计划列表
    val enabledPlans: StateFlow<List<io.github.yuninggu.hrttracker.data.MedicationPlan>> = medicationPlanRepository.getEnabledPlans()
        .stateIn(
            scope = viewModelScope,
            started = SharingStarted.WhileSubscribed(5000),
            initialValue = emptyList()
