...
class WearProductionBypassBoundaryTest {
    ...
    fun `Wear and composition production paths have no persistence bypass`() {
        ...
        val forbidden = listOf(
            ...
            "io.github.yingqiu0871.evolune.data.DoseEventRepository",
            "io.github.yingqiu0871.evolune.data.MedicationPlanRepository",
    ...
    fun `Wear listener uses provider handler and controlled service lifecycle`() {
        val source = Files.readString(
            sourceRoot().resolve("io/github/yingqiu0871/evolune/wear/WearDataLayer.kt")
        )
    ...
    fun `Wear action handler selects only WearActionRecorder`() {
        val source = Files.readString(
            sourceRoot().resolve(
                "io/github/yingqiu0871/evolune/application/WearDoseActionHandler.kt"
            )
        )
