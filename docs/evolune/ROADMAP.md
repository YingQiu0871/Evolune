...
- **Phase-C closure** — **PHASE-C CLOSED — APPROVED**（[`V17_C_PHASE_CLOSURE.md`](v1.7/V17_C_PHASE_CLOSURE.md)）：
  ...
  生产。
- **Phase D · D-01** — contract **APPROVED / FROZEN**；implementation **APPROVED / CLOSED**（final
...
- **Phase D · D-03** — contract **APPROVED / FROZEN**（含 R1/R2，contract HEAD
  `ec0416e32ca0094ce3347a776f76c66b63c0261e`）；implementation **APPROVED / CLOSED**
...
- **Phase D · D-05** — **CLOSED / FROZEN**
  （[`V17_D_05_ACCESSIBILITY_LOCALIZATION_CONTRACT.md`](v1.7/V17_D_05_ACCESSIBILITY_LOCALIZATION_CONTRACT.md)；
  contract @ `b1cdd662fe1bee14beab5cad651aef772f211c68`；final implementation HEAD
  `2a79f1d048905113f53d4be470071a94072c2f96`）：final accessibility/localization hardening of the
  shipped D-04 Timeline surface。CONTRACT — APPROVED / FROZEN；PRODUCTION — IMPLEMENTED /
  APPROVED / CLOSED；EVIDENCE — COMPLETE（186/186 manifest，187 files，0 mismatch）。验收：
  focused JVM 69/0 · full JVM 1490/0 · instrumentation 59/59（Pixel_7 API 35；VisualEvidence = 2）·
  semantics-tree closure（day-cell 单节点 + `Role.Button`；section-header 单短语；MATCHED 双侧）·
...
- **Phase E · contract** — **PHASE-E CONTRACT — APPROVED / FROZEN**
  ...
  V17 PHASE-E CONTRACT R2 — ARCHITECT CLOSURE** + 独立复审 **APPROVE**，P0/P1/P2 = none）：
  E-01 export contract / E-02 CSV v1 schema / E-03 Evolune Portable JSON v1 schema。portable
  truth = dose_events only，**11 portable 字段**（`revision` 为 repository-local concurrency
  metadata；新导入 `revision = 1`）；canonical `schema="evolune-portable"` / `version=1`
  strict；Mahiro v1 = permissive legacy compatibility（无 version gate；E2.6 负向证据）；
  CSV export-only **16 列**（planned_time/timing_delta 恒 empty）；ranges 30/90/all；E1
...
- **v1.7.0 · release packaging / version metadata — RELEASE PACKAGING — APPROVED / READY
  ...
  `a000751d5bf4941819deb367e0c5038fc14117b4`；final independent review **APPROVE V1.7.0
  RELEASE PACKAGING CANDIDATE**，P0/P1/P2 = none；version metadata `UPDATED / VERIFIED`：
...
- **v1.7.4 · 打包候选 — CLOSED / FROZEN · APPROVED**（版本 1.7.4 / Phone 101070400 /
  Wear 1101070400；packaging @ `8e28fce…`；evidence byte-identity correction @ `18e3720…`；
...
- **v1.8.0 · 打包候选 — APPROVED / FROZEN**（版本 1.8.0 / Phone 101080000 /
  Wear 1101080000；`chore(release): prepare v1.8.0 packaging` @ `977afa2…`；独立审批
  D-17：`APPROVE V1.8.0 D-17 — RELEASE PACKAGING FROZEN / READY FOR PUBLICATION`；证书
