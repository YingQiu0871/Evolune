...
**综合评估**: 规格整体聚焦于用户可观察行为，数据字段以业务含义描述。但在 Evolune 数据模型映射、尚未实现的功能引用和少量平台约定的准确性上存在问题。
...
- **置信度**: 高
- **问题类型**: 技术绑定
- **具体问题**: 规格声明 "云端同步与手动导出的本地文件备份（`.hrtbackup` 文件）使用相同的加密格式" 并详细描述 Argon2id + AES-256-GCM 加密。但 `.hrtbackup` 不是 Evolune 的当前公开格式；Evolune 的当前导出格式是 `MahiroJsonFormat`（纯文本 JSON），不是加密的二进制 blob。ARCHITECTURE.md 第 7 阶段的加密备份格式尚未设计。
- **合规/工程风险**: 未经产品和兼容性决策的文件扩展名不应该出现在独立规格中。加密算法选择（Argon2id、AES-256-GCM）也不是单纯的产品行为，而是需要安全审查的实现决策。
- **最小修改建议**: 将 `.hrtbackup` 替换为 "加密备份文件" 描述，去掉具体算法名称 Argon2id 和 AES-256-GCM，替换为 "用户设置的密码保护的加密格式（具体算法待 Evolune 独立评估）"。
- **是否阻止交给 Codex**: 是（文件格式与算法尚未形成 Evolune 决策）
- **需要项目所有者决定**: 否
...
- **置信度**: 中
- **问题类型**: 实现结构暗示
- **具体问题**: FHIR MedicationStatement 字段映射表列出了十分详细的字段对应关系（如 `dosage.doseAndRate.doseQuantity` 包含 "mg、mL、g 或 tablet（含 UCUM 系统编码，视制剂类型而定）"）。这个级别的映射详细度已经接近实现结构。`medicationCodeableConcept` 使用 "Estradiol valerate · Injection" 格式，但该连接符格式不是 FHIR 标准规定。
- **合规/工程风险**: 开发者可能将此映射表直接转化为代码中的字段映射逻辑，在缺少 Evolune 领域模型与互操作测试的情况下过早锁定结构设计。
...
**综合评估**: 这是一份**完整的 cloud sync 功能规格**，覆盖 Google Drive 集成、加密备份、冲突解决和 WorkManager 后台调度。但问题在于：它描述了 ROADMAP 标记为 **P3**、MIGRATION_PLAN 标记为 **Phase 7** 的功能，远超当前开发阶段。它包含大量尚未由 Evolune 决定的实现约定，且与当前架构决策存在严重冲突。可以作为**远期参考**保留，但不能在当前阶段交给实现者。
...
- **置信度**: 高
- **问题类型**: 技术绑定 + 命名污染
- **具体问题**: 与 I-LS-003 相同 —— `.hrtbackup` 文件扩展名和 "Argon2id 密钥派生 + AES-256-GCM 对称加密" 尚未形成 Evolune 的兼容与安全决策。这不是 "可观察产品行为"。用户可观察的是 "文件可以被密码保护"，不是底层加密算法的选择。
...
- **置信度**: 高
- **问题类型**: 技术绑定
- **具体问题**: "密码以 Android Keystore 包装后存储，重启后可直接读取"（第 11 节）把密钥管理实现写成了产品行为。虽然 Android Keystore 是标准 API，但 "exported key material wrapping" 的具体方式（是否导出包装密钥、使用哪些算法参数）属于实现细节。
- **合规/工程风险**: 将具体密钥管理方式描述为产品行为可能限制 Evolune 的安全设计选择（例如使用 AndroidX Security Crypto 库的 EncryptedSharedPreferences 或 MasterKeys API）。
...
- **置信度**: 中
- **问题类型**: 实现结构暗示
- **具体问题**: "Google Drive 上的文件超过大小限制（快照 128 MiB，清单 256 KiB）" —— 128 MiB 和 256 KiB 这些精确数字没有说明是 Google Drive API 限制还是内部限制。如果是内部限制，则是一个实现选择，不应该出现在规格中。
- **合规/工程风险**: 128 MiB 远大于合理用药数据备份的大小——常规用药记录备份可能不超过 1 MB。该值缺少 Evolune 数据规模与设备验证依据。
- **最小修改建议**: 替换为 "单个备份文件有大小上限；超过时同步失败并提示用户" —— 不指定具体数值。
- **是否阻止交给 Codex**: 否
- **需要项目所有者决定**: 是（需要确认这些值来自 Google API 限制还是 Evolune 的容量测试）
...
- **置信度**: 中
- **问题类型**: 视觉设计来源不清
- **具体问题**: 规格列出了极度精确的视觉参数：圆角 22 dp / 10 dp / 999 dp（12.1）、进度条高度 6 dp + 间距 3 dp（12.3）、左强调条 6 dp × 44 dp（12.4）、字体 18 sp / 14 sp（12.4）、预览画布 306 × 276 dp / 624 × 276 dp（12.8）。这些参数没有对应 Evolune 设计系统决策，不应直接转化为生产布局参数。
- **合规/工程风险**: 大量精确视觉参数可能造成不必要的像素级约束，需要由 Evolune 设计审查确认其适配性与来源。
- **最小修改建议**: 在小节开头添加声明："以下数值反映了原参考实现中的视觉规格，Evolune 实现时应根据自身设计系统（`core:designsystem`）调整尺寸比例和间距。所列数值为可观察的品质基准，不意味着必须逐像素复制。"
- **是否阻止交给 Codex**: 否
- **需要项目所有者决定**: 是（需要设计审查确认这些视觉参数的来源和适配性）
...
- **置信度**: 高
- **问题类型**: 命名污染 + 优先级冲突
- **具体问题**: 全文反复使用 "Anchor" 作为 "Tracked Date" 的备选名称，但该词不是 Evolune 已确认的面向用户功能名称。更严重的是，整份规格描述了一个 `P2（1.0，非 MVP）` 功能，但文件结构与 phone-widgets.md 并列，没有级别标注。与 `ARCHITECTURE.md` ADR-011 "在产品所有者确认前，统一标记为 P2（1.0，待产品确认，非 MVP），Phase 1 不创建实体或迁移" 矛盾。
...
- **置信度**: 高
- **问题类型**: 品牌资产风险
- **具体问题**: "渐变彩旗背景"包含 "彩虹旗、跨性别旗等共 9 种，外加'无'"，但规格没有说明这些图案的具体渐变、角度和形状应由 Evolune 设计系统定义，也没有给出资产来源记录。
- **合规/工程风险**: Evolune 如需提供同等功能，应独立设计旗帜图案，不能从来源未经核验的图片或 SVG 资源中提取色值和渐变定义。
- **最小修改建议**: 在"渐变彩旗背景"描述中添加："旗帜图案的具体设计（颜色、渐变、形状）应由 Evolune 独立创作，并记录所用资产来源。"
- **是否阻止交给 Codex**: 否
- **需要项目所有者决定**: 否
...
- **置信度**: 中
- **问题类型**: 架构集成描述不足
- **具体问题**: "所有外观参数……均为全局共享设置，修改后对该应用所有小组件类型（追踪日期小组件和服药进度小组件）统一生效"。这描述了一个跨小组件类型共享的配置存储。当前 Evolune 没有小组件外观配置存储，DataStore 中的 `UserSettings` 包含 `themeMode` 和 `colorTheme` 但这不是小组件级的外观定制。该完整外观系统尚未由 Evolune 的 `core:designsystem` 定义。
- **合规/工程风险**: 实现者可能直接在 DataStore 中创建全部外观键，引入不必要的数据模型膨胀。
...
- **置信度**: 中
- **问题类型**: 实现结构暗示
- **具体问题**: "25 个采样点、每点间隔 2 小时的浓度历史曲线" —— 25 个点 × 2 小时间隔 = 48 小时。不同采样策略会产生不同的曲线平滑度：Evolune 可以选择 97 个点 × 30 分钟 = 48 小时，或其他采样密度。将 25 个点作为规格会不必要地限制采样方案。
...
- **置信度**: 中
- **问题类型**: 技术绑定
- **具体问题**: "手表端缓存快照使用 AES-256-GCM 加密，密钥存储于手表的 Android Keystore"。与 I-LS-009 相同 —— 虽然 Android Keystore 是标准 API，选择 AES-256-GCM 而非其他 AEAD 算法不是用户可观察的产品行为。手表端加密需求是合理的（protect health data at rest），但算法选择应该留给 Evolune 的安全工程决策。
...
**综合评估**: 描述了手机与手表之间的协议格式和同步流程。整体以行为描述为主，但个别字段的枚举值列表和采样策略细节过度约束实现结构。
...
- **置信度**: 中
- **问题类型**: 实现结构暗示
- **具体问题**: 协议字段列表包含 "最多约 64 条"（计划行数限制）、"最多约 97 个点"（血药浓度采样点数）、"约 256 KB"（快照大小上限）。这些精确限制没有对应 Evolune 的 Wear Data Layer 容量测试依据。Wearable Data Layer 的官方建议是 DataItem 小于 100KB，因此 256KB 尤其需要验证。
- **合规/工程风险**: Evolune 的快照复杂度可能变化，硬编码未经验证的限制会导致过早优化或容量不足。
...
- **置信度**: 低
- **问题类型**: 实现结构暗示
- **具体问题**: 协议定义了三种命令类型：记录服药、跳过本次、撤销服药。这是合理的协议语义。规格还要求每条命令包含唯一请求 ID，供手机端去重和幂等处理，但没有约束请求 ID 的具体生成方式，因此保持在行为契约层面。
- **合规/工程风险**: 低。请求 ID 幂等是分布式系统的通用模式。
- **最小修改建议**: 无需修改。
- **是否阻止交给 Codex**: 否
- **需要项目所有者决定**: 否
...
| `wear-protocol.md` | **修改后可用** | 无 | 精确限制值缺少容量测试依据 |
...
- **问题**: 三份文件分别在不同上下文中提到 "Argon2id + AES-256-GCM" 作为加密方案（云同步备份、Wear 缓存加密、Health Connect 关联格式）。虽然 AES-GCM 和 Argon2 是现代加密方案，但将具体算法组合写入行为规格会过早锁定安全实现。Evolune 的安全工程审计应独立选择算法组合和参数。
