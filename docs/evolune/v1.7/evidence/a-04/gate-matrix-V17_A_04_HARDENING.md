...
| # | 结果 |
|---|---|
| 1.1 | `V17_A_03_HISTORY_UI.md` §18 的 key 已改为 `history_note_inferred_match`（并注明更名来源） |
...
- 证据目录：`docs/evolune/v1.7/evidence/a-04/`：gate matrix（本文件副本）、fresh JVM XML/log、
  targeted XML/log、full XML/log、`explain-query-plan.txt`、zero-write / undo / refresh 证据指针、
  聚合 TSV、assemble log、`source-diff-stat.txt`、`MANIFEST.sha256`
  （coverage 100% + `sha256sum -c` + self hash + HEAD blob == worktree）；
